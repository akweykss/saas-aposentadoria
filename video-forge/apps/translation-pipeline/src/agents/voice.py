    ...
    async def process(self, job_id: str) -> Optional[JobManifest]:
        ...
        try:
            ...
            if tts_ok:
                ...
                # ── Step 5: Generate subtitle segments from text ───────
                # Split the translated text into sentences and distribute
                # timestamps proportionally across the trimmed audio.
                _progress(68, "📝 Gerando legendas por frases...")
                ...
                # Apply pre-roll: shift subtitles 0.4s earlier so they
                # appear slightly before the audio (cinema standard).
                # This compensates for proportional distribution drift
                # and gives viewers time to start reading.
                PRE_ROLL_S = 0.4
                for sub in subtitle_segments:
                    sub["start"] = round(max(0.0, sub["start"] - PRE_ROLL_S), 3)
                    sub["end"] = round(max(0.1, sub["end"] - PRE_ROLL_S), 3)

                logger.info(
                    "voice.subtitles_generated",
