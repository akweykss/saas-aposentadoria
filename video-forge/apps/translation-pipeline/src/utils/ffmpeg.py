...
class FFmpegWrapper:
    ...
    async def full_synthesis_pipeline(
        ...
        if use_filter_complex:
            filter_graph = self._build_filter_complex(
                sub_blur_region=sub_blur_region or {"y": 900, "h": 180},
    ...
    def _build_filter_complex(
        ...
        """Build a filter_complex graph string with gblur and optional per-segment sync.
        ...
        When segment_speeds is provided, builds a per-segment speed graph:
            ...
            [synced] → hflip → split → crop+gblur → overlay → lut3d → noise → [vout]

        When no segment_speeds, uses global speed_factor:
            [0:v] → setpts → hflip → split → blur → overlay → lut3d → noise → [vout]
        """
        blur_y = sub_blur_region.get("y", 900)
        blur_h = sub_blur_region.get("h", 180)

        # Add padding around the blur region for a softer edge
        pad = 15
        ...
        if apply_hflip:
            ...
            split_input = pre_input

        # ── Stage 3: split → crop → gblur → overlay ──────────────
        parts.append(f"{split_input}split=2[main][blur_src]")
        parts.append(
            f"[blur_src]crop=iw:{crop_h}:0:{crop_y},"
            f"gblur=sigma=60[blurred]"
        )
        parts.append(
            f"[main][blurred]overlay=0:{overlay_y}[with_blur]"
        )
        ...
        if post_filters:
            ...
            parts.append(f"[with_blur]{post_chain}[vout]")
        else:
            # No post-processing — just rename the label
            parts.append("[with_blur]null[vout]")
